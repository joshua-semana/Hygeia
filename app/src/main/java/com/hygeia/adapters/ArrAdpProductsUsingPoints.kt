...
class ArrAdpProductsUsingPoints(
    ...
    private val clickListener: ActPurchaseUsingStars
    ) : RecyclerView.Adapter<ArrAdpProductsUsingPoints.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (product != null) {
            with(holder) {
                with(product) {
                    nameAndPrice.text = "$Name (🌟${formatPoints(Points!!)})"
